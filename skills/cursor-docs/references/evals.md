...
Use the [Cursor SDK](https://cursor.com/docs/sdk/typescript.md) to run Cursor's agent loop inside your own eval harness. The same agent that powers the Cursor IDE, CLI, and web app is scriptable from TypeScript, so you can score Grok 4.7 (and other models we support) on your benchmarks.
...
- **Grok-first, multi-model.** Grok 4.7 is the default to evaluate, but any model in Cursor's catalog works through the same API.
...
A single-task eval against Grok 4.7 on a local working tree:
...
```typescript
const models = ["grok-4.7", "composer-2.5", "gpt-5.6-sol", "claude-opus-5", "gemini-3.1-pro"];
...
- Read about [Grok 4.7](https://cursor.com/docs/models/grok-4-7.md) and the rest of Cursor's [models](https://cursor.com/docs/models-and-pricing.md).
